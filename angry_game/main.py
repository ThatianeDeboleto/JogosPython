...
walls = Message(164, 145, 80, "Deboleto",title_font, WHITE, win)

tap_to_play_font = "Fonts/DebugFreeTrial-MVdYB.otf"
tap_to_play = Message(144, 400, 32, "TOQUE PARA JOGAR",tap_to_play_font, WHITE, win)
tap_to_replay = Message(144, 400, 30, "Toque para repetir",tap_to_play_font, WHITE, win)
...
while running:
	...
	if home_page:
		...
		if bird_dead and len(destruct_group) == 0:
			...
			if score < high_score:
				score_msg = Message(144, 60, 55, "Pontos",font, WHITE, win)
			else:
				score_msg = Message(144, 60, 55, "Nova pontuação",font, WHITE, win)
